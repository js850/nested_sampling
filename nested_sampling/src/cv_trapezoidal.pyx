...
def compute_cv_c(np.ndarray[double, ndim=1, mode="c"] E_list,
    ...
    heat_capacity_loop(<double*>E_list.data,<long double*>dos_list.data,<double*>cv_list.data, N, Tmin, Tmax, nT, ndof)
...
def compute_cv_c_single(np.ndarray[double, ndim=1, mode="c"] E_list, 
    ...
    if (P == 1) or (imp == 0): 
        ...
        compute_dos_imp(<long double*>dos_list.data, N, P, K)
    return heat_capacity(<double*>E_list.data,<long double*>dos_list.data, N, T, ndof)
